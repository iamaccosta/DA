 ...
 * A simple implementation of mutable priority queues, required by Dijkstra2 algorithm.
 *
...
#ifndef FEUP_CAL_PROJ_MUTABLEPRIORITYQUEUE_H
#define FEUP_CAL_PROJ_MUTABLEPRIORITYQUEUE_H
...
template<class T>
bool MutablePriorityQueue<T>::found(T *x) {
    for (auto v : H)
        if (v == x)
            return true;
    return false;
...
#endif //FEUP_CAL_PROJ_MUTABLEPRIORITYQUEUE_H